...
#ifdef UCOSII
OS_EVENT *sem_can1 = NULL;
#endif

void CAN1_config_init(long baud)
{
...
void can12_recv_test(void)
{
	...
	while(1)
	{
		cnt = GetCan1Queue(&rx, 10);
		if (cnt > 0)
		{
			CAN2_tx_msg((CanTxMsg*)&rx);
		}

		cnt = GetCan2Queue(&rx, 10);
		if (cnt > 0)
		{			
